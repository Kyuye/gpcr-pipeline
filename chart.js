...
<defs>
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLS; col++) {
      ...
      if (chartType === 'gpcr') svg += panelGpcr(g, dataMap[g], ox, oy, pw, ph, includeCtrl, labelMap[g]);
      else if (chartType === 'opto-other') svg += panelOptoSimple(g, dataMap[g], ox, oy, pw, ph, labelMap[g]);
...
/* ============================================================
   ...
   ============================================================ */
function panelGpcr(gpcr, recData, ox, oy, pw, ph, includeCtrl, graphLabel) {
  let s='';
  ...
  const nPG=includeCtrl?6:4, totalBars=nPG*2, gapMid=2.5;
  const step=(plotW-gapMid)/totalBars, barW=step*0.72;
  ...
  const barX=idx=>pl+idx*step+(idx>=nPG?gapMid:0);
  const barCX=idx=>barX(idx)+step/2;

  // Background bands
  if(includeCtrl){[0,nPG].forEach(b=>{s+=`<rect x="${barX(b+2)-step*0.14}" y="${pt}" width="${step*2+step*0.28}" height="${plotH}" fill="#EBEBEB"/>`;s+=`<rect x="${barX(b+4)-step*0.14}" y="${pt}" width="${step*2+step*0.28}" height="${plotH}" fill="#F8DFB1"/>`;});}
  else{[0,nPG].forEach(b=>{s+=`<rect x="${barX(b)-step*0.14}" y="${pt}" width="${step*2+step*0.28}" height="${plotH}" fill="#EBEBEB"/>`;s+=`<rect x="${barX(b+2)-step*0.14}" y="${pt}" width="${step*2+step*0.28}" height="${plotH}" fill="#F8DFB1"/>`;});}

  // LNC divider
  s+=`<line x1="${barX(nPG)-gapMid/2}" y1="${pt}" x2="${barX(nPG)-gapMid/2}" y2="${pb}" stroke="#BCBCBB" stroke-width="0.088" stroke-dasharray="0.8,0.5"/>`;
  ...
  const lY=xLY+2.0;
  const l1L=barX(0)+step*0.1,l1R=barX(nPG-1)+step*0.9,l2L=barX(nPG)+step*0.1,l2R=barX(totalBars-1)+step*0.9;
...
/* ============================================================
   ...
   ============================================================ */
function panelOptoSimple(gpcr, recData, ox, oy, pw, ph, graphLabel) {
  let s='';
  const pl=ox+5.8,pr=ox+pw-0.6,pt=oy+1.0,pb=oy+ph-10.0;
  const plotW=pr-pl,plotH=pb-pt; if(plotH<=0)return s;

  const platforms=['LNC1.0','LNC2.0'];
  // Gather all conditions from data → each gets a Dark+Blue pair per LNC
  let condNames=[];
  if(recData){const fp=Object.keys(recData)[0];if(fp&&recData[fp])condNames=Object.keys(recData[fp].conditions);}
  const nPerCond=2; // dark+blue
  const nPG=condNames.length*nPerCond||2; // bars per LNC group
  const totalBars=nPG*2;
  const gapMid=2.5;
  const step=(plotW-gapMid)/totalBars,barW=step*0.72;
  ...
  // Bars
  bars.forEach(bar=>{const x=barX(bar.idx)+(step-barW)/2,bH=ym>0?(bar.value/ym)*plotH:0;if(bH>0)s+=`<rect x="${x}" y="${pb-bH}" width="${barW}" height="${bH}" fill="${bar.type==='dark'?'url(#gD)':'url(#gB)'}" stroke="#444" stroke-width="0.088"/>`;});
